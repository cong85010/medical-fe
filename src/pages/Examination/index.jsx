...
import { Table, Button, Form, Input, Select, Space, Tag, Descriptions, Modal } from 'antd';
...
const ExaminationPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchExaminationHistory = async () => {
      if (selectedPatient) {
        try {
          const response = await fetch(`https://api.example.com/examination-history/${selectedPatient.id}`);
          const data = await response.json();
    ...
    },
    {
      ...
      render: (record) => (
        <Space>
          ...
          <Button type="primary" onClick={() => handlePrescribe(record)} disabled={!selectedPatient}>
            Prescribe
          </Button>
        </Space>
      ),
  ...
  return (
    <div>
      ...
      <Button type='primary' onClick={() => setIsExaminationModalVisible(true)}>Kết quả khám bệnh</Button>

      <ExaminationFormModal isExaminationModalVisible={isExaminationModalVisible} onCancel={handleCancel}/>
    </div>
  );
};
